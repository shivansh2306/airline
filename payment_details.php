<?php
	session_start();
	 if($_SESSION['login_user']==null||$_SESSION['type']!='payment'){
	...
	<body>
		...
		<form action="payment_details_form_handler.php" method="post">
			...
			<?php
				...
				{	
					$query="SELECT price_economy,seats_economy,j.total_capacity FROM flight_details as f INNER JOIN jet_details as j ON f.jet_id=j.jet_id where flight_no=? and departure_date=?";
					$stmt=mysqli_prepare($dbc,$query);
					mysqli_stmt_bind_param($stmt,"ss",$flight_no,$journey_date);
					mysqli_stmt_execute($stmt);
					mysqli_stmt_bind_result($stmt,$ticket_price,$seats_economy,$total_capacity);
					mysqli_stmt_fetch($stmt);
					//echo $flight_no;
					$ticket_price=($ticket_price+30*($total_capacity-$seats_economy));
				}
				else if($_SESSION['class']=='business')
				{
					$query="SELECT price_business,seats_business,j.total_capacity FROM flight_details as f INNER JOIN jet_details as j ON f.jet_id=j.jet_id where flight_no=? and departure_date=?";
					$stmt=mysqli_prepare($dbc,$query);
					mysqli_stmt_bind_param($stmt,"ss",$flight_no,$journey_date);
					mysqli_stmt_execute($stmt);
					mysqli_stmt_bind_result($stmt,$ticket_price,$seats_business,$total_capacity);
					mysqli_stmt_fetch($stmt);
					$ticket_price=($ticket_price+30*($total_capacity-$seats_business));
				}
