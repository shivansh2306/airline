	...
	<body>
		<?php
			...
			if(isset($_POST['Submit']))
			{
				...
				{	
					$query="SELECT price_economy,seats_economy,j.jet_type FROM flight_details as f INNER JOIN jet_details as j ON f.jet_id=j.jet_id where flight_no=? and departure_date=?";
					$stmt=mysqli_prepare($dbc,$query);
					mysqli_stmt_bind_param($stmt,"ss",$flight_no,$journey_date);
					mysqli_stmt_execute($stmt);
					mysqli_stmt_bind_result($stmt,$ticket_price,$seats_economy,$jet_type);
					mysqli_stmt_fetch($stmt);
					
				}
				else if($_SESSION['class']=='business')
				{
					$query="SELECT price_business,seats_business,j.jet_type FROM flight_details as f INNER JOIN jet_details as j ON f.jet_id=j.jet_id where flight_no=? and departure_date=?";
					$stmt=mysqli_prepare($dbc,$query);
					mysqli_stmt_bind_param($stmt,"ss",$flight_no,$journey_date);
					mysqli_stmt_execute($stmt);
					mysqli_stmt_bind_result($stmt,$ticket_price,$seats_business,$jet_type);
					mysqli_stmt_fetch($stmt);
				}
